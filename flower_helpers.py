...
def train(
    ...
    privacy_engine = PrivacyEngine(
        ...
        alphas=alphas,
        noise_multiplier=nm,
        max_grad_norm=[0.671293256841709, 0.534522, 0.534522, 0.534522, 0.583733266818877,
                       0.534522, 0.534522, 0.534522, 0.525359940136989, 0.534522,
                       0.525359940136989, 0.534522, 0.291866633409439, 0.534522],
        target_delta=delta,
        clipping='per_layer',
    )
    ...
    gradients = {name: [] for name, module in net.named_modules() if isinstance(module, (nn.Conv2d, nn.Linear))}
        ...
        for name, module in net.named_modules():
            if isinstance(module, (nn.Conv2d, nn.Linear)):
                for param_name, param in module.named_parameters():
                    if param.grad is not None and 'weight' in param_name:
    ...
    for name, gradients in gradients.items():
        if gradients:
            average_gradient = sum(gradients) / len(gradients)
            avg_gradients[name].append(average_gradient)
        ...
        return_dict["avg_gradients"] = avg_gradients
        return_dict["track_epsilon"] = track_epsilon
        return_dict["track_accuracy"] = track_accuracy
